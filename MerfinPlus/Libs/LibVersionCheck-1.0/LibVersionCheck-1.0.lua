...
--- @param addonName string
--- @return table<string, string> # [playerName] = version
--- @public
...
function LibVersionCheck:OnVersionChanged(sender, addonName, version, previousVersion)
    ...
    for _, callback in ipairs(callbacks) do
        pcall(callback, sender, addonName, version, previousVersion);
    end
...
function e:PLAYER_GUILD_UPDATE(unit)
    ...
    timer:ScheduleTimer(function()
        ...
        if LibVersionCheck.isInGuild then
            ...
            LibVersionCheck.playerGuild = guildName;
...
function e:GROUP_JOINED()
  local channel = getGroupChannel()
  if channel then
    if not LibVersionCheck._lastWasInGroup then
      -- GROUP_JOINED
      LibVersionCheck:SendVersion(channel)
    end
    -- GROUP_JOINED already
    LibVersionCheck._lastWasInGroup = true
  else
    -- GROUP_LEFT
    LibVersionCheck._lastWasInGroup = false
  end
end
